create table department (                      --학과
    dep_no NUMBER primary key,             --학과번호
...
ALTER TABLE department
ADD CONSTRAINT uk_department_dep_name
UNIQUE (dep_name);
...
CREATE SEQUENCE professor_pro_seq_seq START WITH 1 MAXVALUE 9999 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER professor_pro_seq_trg
BEFORE INSERT ON professor
FOR EACH ROW
BEGIN
  IF :NEW.pro_seq IS NULL THEN
    :NEW.pro_seq := professor_pro_seq_seq.NEXTVAL;
  END IF;

  -- 10자리 교수번호 = YYYY(4) * 10^6 + 학과(2) * 10^4 + 순번(4)
  :NEW.pro_no := (EXTRACT(YEAR FROM :NEW.appointment_date) * 1000000)
               + (:NEW.dep_no * 10000)
               +  :NEW.pro_seq;
END;
/
