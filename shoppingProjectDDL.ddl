...
CREATE TABLE cart (
    ...
    cartprodcount INTEGER NOT NULL,
...
CREATE TABLE product (
    ...
    prodsold     INTEGER NOT NULL,
    proddiscount INTEGER NOT NULL
);
...
insert into "User" values ('user7', '강감찬', '1984-06-15', 'F', '010-4101-1984', 'kang@daum.com', 3610, 0, '대구', '2022-01-10 10:50:12');
...
insert into Category values (17, '컴퓨터·가정·디지털');
insert into Category values (18, '여행');
insert into Category values (19, '도서');

insert into Product values (100101, 11, 10003, '반팔티 L~XL', 869, 25000, 132, 20);
...
insert into OrdersItem values (11, '22010510021', 110101, 25000, 20, 1);
...
insert into Cart values (1, 'user1', 110101, 1, '2022-01-10 10:50:12');
...
-- 문제 2
select p.prodno, c.catename, p.prodname, p.prodprice, s.company, s.tel
from product p
join category c on c.cateno = p.cateno
join seller s on s.sellerno = p.sellerno;

-- 문제 3
select u.userid, u.name, u.hp, u.point, CASE 
    WHEN SUM(p.point) IS NULL THEN 0 
    ELSE SUM(p.point) 
  END as "적립포인트 총합"
from "User" u
...
-- 문제 4
select o.orderno, o.userid, u.name, o.ordertotalprice, orderdate
from orders o
...
select 
    ...
    o.userid, 
    u.name,
    LISTAGG(p.prodname, ',') within group (order by p.prodname), 
    o.orderdate
from orders o
join "User" u on u.userid = o.userid
...
-- 문제 6
select prodno, prodname, prodstock as 가격, proddiscount, prodstock*(100-proddiscount)/100 as "할인된 가격"
from product;

-- 문제 7
select p.prodno, p.prodname, p.prodstock as 가격, p.prodprice as 재고수량, s.company
from product p
join seller s on s.sellerno = p.sellerno
where manager='고소영';
...
select
    orderno as 주문번호,
    sum(itemprice*(100-itemdiscount)/100*itemcount) as 최종총합
from ordersitem
group by orderno
having sum(itemprice*(100-itemdiscount)/100*itemcount)>=100000
order by 최종총합 desc;
